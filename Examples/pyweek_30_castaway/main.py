...
level.water.color = color.color(160, 1, 0.8, 0.5)
level.water.enabled = False
Entity(
    model="plane",
    position=level.water.position,
    scale=9999,
    color=color.color(160, 1, 0.8, 0.5),
    double_sided=True,
)
scene.fog_color = color.color(6, 0.1, 0.85)
...
def open_chest():
    if distance_xz(player.position, level.chest.position) < 6:
        level.chest_lid.animate(
            "rotation_x", level.chest_lid.rotation_x + 120, duration=0.2
        )
        invoke(setattr, level.bow, "enabled", True, delay=0.25)
...
def input(key):
    ...
    if level.bow.enabled and key == "left mouse down":
        player.arrow = duplicate(
            level.arrow,
            world_parent=level.bow,
            position=Vec3(-0.2, 0, 0),
            rotation=Vec3(0, 0, 0),
        )
        player.arrow.animate(
            "position",
            player.arrow.position + Vec3(0, 0, -2),
            duration=0.2,
            curve=curve.linear,
        )
    ...
    if level.bow.enabled and key == "left mouse up":
        if mouse.hovered_entity and mouse.hovered_entity.visible:
            ...
            if mouse.hovered_entity == level.eye_trigger:
                invoke(open_gate, delay=0.3)
                destroy(player.arrow, delay=0.1)
                return
            ...
            player.arrow.animate(
                "position",
                player.arrow.world_position + (player.arrow.forward * 100),
                0.5,
                curve=curve.linear,
                interrupt="kill",
            )
...
def update():
    if (
        not level.bow.enabled
        and mouse.hovered_entity in (level.chest, level.chest_lid)
        and distance_xz(player.position, level.chest.position) < 6
    ):
        level.chest.color = color.color(90, 0.4, 0.8)
        level.chest_lid.color = color.color(90, 0.4, 0.8)
    else:
...
def open_gate():
    ...
    level.gate.animate_position(
        level.gate.position + (level.gate.left) * 10, duration=5, curve=curve.linear
    )
    level.gate_001.animate_position(
        level.gate_001.position + (level.gate_001.right) * 10,
        duration=5,
        curve=curve.linear,
    )
